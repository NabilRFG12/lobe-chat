...
const AskAgentCommands = memo(() => {
  ...
  // Check if AGENT matches the query
  const showAgent = !mentionQuery || 'agent'.includes(mentionQuery);
    ...
    <Command.Group heading={t('cmdk.mentionAgent')}>
      ...
      {showAgent && (
        <Command.Item
          ...
          onSelect={() => handleAgentSelect(inboxAgentId, 'AGENT', DEFAULT_INBOX_AVATAR)}
        >
          <Avatar emojiScaleWithBackground avatar={DEFAULT_INBOX_AVATAR} shape="square" size={18} />
          <div className={styles.itemContent}>
            <div className={styles.itemLabel}>@AGENT</div>
          </div>
        </Command.Item>
      )}
